




class Node(object):
    def __init__(self, start, end):
        self.start = start
        self.end = end
        self.total = 0
        self.left = None
        self.right = None
        

class NumArray(object):
    def __init__(self, nums):
        """
        initialize your data structure here.
        :type nums: List[int]
        """
        #helper function to create the tree from input array
        def createTree(nums, l, r):
            
            #base case
            if l > r:
                return None
                
            #leaf node
            if l == r:
                n = Node(l, r)
                n.total = nums[l]
                return n
            
            mid = (l + r) // 2
            
            root = Node(l, r)
            
            #recursively build the Segment tree
            root.left = createTree(nums, l, mid)
            root.right = createTree(nums, mid+1, r)
            
            #Total stores the sum of all leaves under root
            #i.e. those elements lying between (start, end)
            root.total = root.left.total + root.right.total
                
            return root
        
        self.root = createTree(nums, 0, len(nums)-1)
            
    def update(self, i, val):
        """
        :type i: int
        :type val: int
        :rtype: int
        """
        #Helper function to update a value
        def updateVal(root, i, val):
            
            #Base case. The actual value will be updated in a leaf.
            #The total is then propogated upwards
            if root.start == root.end:
                root.total = val
                return val
        
            mid = (root.start + root.end) // 2
            
            #If the index is less than the mid, that leaf must be in the left subtree
            if i <= mid:
                updateVal(root.left, i, val)
                
            #Otherwise, the right subtree
            else:
                updateVal(root.right, i, val)
            
            #Propogate the changes after recursive call returns
            root.total = root.left.total + root.right.total
            
            return root.total
        
        return updateVal(self.root, i, val)

    def sumRange(self, i, j):
        
        sum of elements nums[i..j], inclusive.
        :type i: int
        :type j: int
        :rtype: int
        
        #Helper function to calculate range sum
        def rangeSum(root, i, j):
            
            #If the range exactly matches the root, we already have the sum
            if root.start == i and root.end == j:
                return root.total
            
            mid = (root.start + root.end) // 2
            
            #If end of the range is less than the mid, the entire interval lies
            #in the left subtree
            if j <= mid:
                return rangeSum(root.left, i, j)
            
            #If start of the interval is greater than mid, the entire inteval lies
            #in the right subtree
            elif i >= mid + 1:
                return rangeSum(root.right, i, j)
            
            #Otherwise, the interval is split. So we calculate the sum recursively,
            #by splitting the interval
            else:
                return rangeSum(root.left, i, mid) + rangeSum(root.right, mid+1, j)
        
        return rangeSum(self.root, i, j)

class Solution:
    def longestRepeating(self, s: str, queryCharacters: str, queryIndices: List[int]) -> List[int]:

        hm = defaultdict(None)
        
        #numArray = NumArray(nums)
# numArray.sumRange(0, 1)
# numArray.update(1, 10)
# numArray.sumRange(1, 2)
        #c = nums[]
        
        se = set(s)
        for c in [chr(i + ord('a')) for i in range(26)]:
            #if c in se:
            n = list(map(lambda x: 1 if x == c else 0, s))
            #print(n)
            hm[c] = NumArray(n)
            
            
        #for c in s:
            #if c not in hm:
            #    n = list(map(lambda x: 1 if x == c else 0, s))
            #    print(n)
            #    hm[c] = NumArray(n)
            #hm[c].addRange(i,i)
        
        #print(hm)
        res = []

        for i,c in enumerate(queryCharacters):
            
            j = queryIndices[i]
            #print(i, j, s[i])
            
            if j > 0:
                left = hm[s[i]].sumRange(j-1,j-1)

            if j <len(s) -1:
                right = hm[s[i]].sumRange(j+1,j+1)

            hm[s[i]].update(j,0)

            
            if j > 0:
                left = hm[h].sumRange(j-1,j-1)
            
            if j <len(s) -1:
                right = hm[h].sumRange(j+1,j+1)

            hm[c].update(j,1)
            
            #s = s[:i] + c + s[i+1:]

            m = 0
            #for h in hm:
            m = max(m, hm[h].sumRange(0,len(s)-1) )
            res.append(m)
        
        return res






#from segment_tree import *


from sortedcontainers import SortedList

class Solution:
    def longestRepeating(self, s: str, queryCharacters: str, queryIndices: List[int]) -> List[int]:
        sl = SortedList()
        length = SortedList()
        curr = 0
        for char, it in itertools.groupby(s):
            c = sum(1 for _ in it)
            length.add(c)
            sl.add((curr, curr + c, char, c))
            curr += c
        ans = []
        #print(sl)
        #print(length)
        for char, i in zip(queryCharacters, queryIndices):
            t = (i, math.inf, 'a')
            print(t)
            print(sl)
            index = sl.bisect_right(t[0]) - 1
            #print(index)
            to_remove = [sl[index]]
            to_add = []
            left, right, original_char, l = to_remove[0]
            if original_char != char:
                length.remove(right - left)
                if right - left > 1:
                    if i == left:
                        left += 1
                        to_add.append((left, right, original_char))
                        length.add(right - left)
                    elif i == right - 1:
                        right -= 1
                        to_add.append((left, right, original_char))
                        length.add(right - left)
                    else:
                        to_add.append((left, i, original_char))
                        length.add(i - left)
                        to_add.append((i + 1, right, original_char))
                        length.add(right - (i + 1))
                
                l, r = i, i + 1
                if index - 1 >= 0 and sl[index - 1][1:3] == (i, char):
                    l, old_r, _, o = sl[index - 1]
                    to_remove.append(sl[index - 1])
                    length.remove(old_r - l)
                if index + 1 < len(sl) and sl[index + 1][0] == i + 1 and sl[index + 1][2] == char:
                    old_l, r, old_length, o = sl[index + 1]
                    to_remove.append(sl[index + 1])
                    length.remove(r - old_l)
                length.add(r - l)
                sl.add((l, r, char))
                for t in to_remove:
                    sl.remove(t)
                sl.update(to_add)
            # print(sl)
            # print(length)
            ans.append(length[-1])

        return ans


